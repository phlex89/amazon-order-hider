  ...
  <div class="popup-container">
    ...
    <div class="hidden-orders">
      <div class="hidden-orders-header">
        Ordini nascosti
        <span v-if="hiddenOrders.length > 0" class="count">({{ hiddenOrders.length }})</span>
      </div>
...
async function loadData(): Promise<void> {
  try {
    ...
    hiddenOrders.value = data.hiddenOrders.sort((a, b) => 
...
async function handleRestoreOrder(orderId: string): Promise<void> {
  try {
    ...
    hiddenOrders.value = hiddenOrders.value.filter(order => order.id !== orderId)
  } catch (error) {
...
async function handleClearAll(): Promise<void> {
  if (!window.confirm('Sei sicuro di voler ripristinare tutti gli ordini nascosti?')) {
    return
  }
